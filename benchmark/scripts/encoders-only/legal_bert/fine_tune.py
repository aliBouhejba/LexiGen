...
MAX_SEQ_LEN = 512
PAIR_TEMPLATE = "Key point: {keypoint}"
EPOCHS = 6
...
def encode_pair(tokenizer, segment: str, keypoint_text: str, gold_label: str):
    pair = PAIR_TEMPLATE.format(keypoint=keypoint_text)
    enc = tokenizer(segment, pair, truncation=True, max_length=MAX_SEQ_LEN)
    enc["labels"] = LABEL2ID[gold_label]
    return enc
...
class PairsDataset(torch.utils.data.Dataset):
    def __init__(self, df: pd.DataFrame, tokenizer):
        ...
        self.enc = [encode_pair(tokenizer, self.df.iloc[i]["segment"], self.df.iloc[i]["keypoint_text"], self.df.iloc[i]["gold_label"]) for i in range(len(self.df))]
...
def main() -> None:
    ...
    train_dataset = PairsDataset(train_pairs_df, tokenizer)
    eval_dataset = PairsDataset(test_pairs_df, tokenizer)
    ...
    y_pred = [ID2LABEL[int(x)] for x in test_preds_ids]
