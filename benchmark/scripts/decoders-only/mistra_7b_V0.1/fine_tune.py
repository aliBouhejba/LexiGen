import os, sys, types, warnings, importlib.machinery, json, gc, glob, re
import numpy as np, numpy.core.numerictypes as _npt
if not hasattr(np, "dtypes"): np.dtypes = _npt
def _stub(name: str):
    m = types.ModuleType(name); m.__spec__ = importlib.machinery.ModuleSpec(name, None); sys.modules[name] = m; return m
jax = _stub("jax"); jax.numpy = _stub("jax.numpy"); jax.core = _stub("jax.core"); jaxlib = _stub("jaxlib"); flax = _stub("flax"); flax.linen = _stub("flax.linen"); jax.version = _stub("jax.version"); jax.version.__file__ = "fake_jax_version.py"
class _FakeTracer: ...
jax.core.Tracer, jax.numpy.ndarray = _FakeTracer, np.ndarray
...
OUT_DIR = "/content/compliance_Model"
EVAL_OUT_DIR = "/content/outputs"
FMT_TRAIN_JSON = "/content/compliance_train.json"
BASE_MODEL = "mistralai/Mistral-7B-v0.1"
SEGMENTS_FILES = ["/content/raw_eval.json"]
KEYPOINTS_FILES = ["/content/GDPR_articles_obligations_keypoints_organized.json"]
BATCH_SIZE, GRAD_ACC_STEPS = 6, 3
NUM_EPOCHS, LEARNING_RATE = 6, 2e-4
MAX_SEQ_LEN, LOGGING_STEPS = 2400, 1
...
def load_train_dataset():
    if not os.path.exists(FMT_TRAIN_JSON): raise FileNotFoundError(FMT_TRAIN_JSON)
    data = json.load(open(FMT_TRAIN_JSON, "r", encoding="utf-8"))
    if not isinstance(data, list) or not data: raise ValueError("training data must be a non-empty list")
    return Dataset.from_list(data)

def train_lora_return_last_ckpt():
    train_ds = load_train_dataset()
    base, tok = load_model_tok_for_train()
    peft_cfg = LoraConfig(r=64, lora_alpha=64, lora_dropout=0.05, bias="none", task_type="CAUSAL_LM", target_modules=["v_proj","o_proj","q_proj","down_proj","k_proj","up_proj"])
    cfg = SFTConfig(
        output_dir=OUT_DIR,
    ...
    trainer = SFTTrainer(model=base, tokenizer=tok, train_dataset=train_ds, args=cfg, formatting_func=lambda ex: [f"{ex['instruction']}\n\n{ex['output']}\n"], peft_config=peft_cfg)
    hf_logging.set_verbosity_error()
    trainer.train()
    free_cuda()
    ckpts = sorted(glob.glob(os.path.join(OUT_DIR, "checkpoint-*")), key=os.path.getmtime)
    if not ckpts: raise RuntimeError("no checkpoints saved")
    return ckpts[-1]
...
def load_segments_flat(files: List[str]) -> pd.DataFrame:
    ...
    if df.empty: raise ValueError("no segments")
...
def load_keypoints(files: List[str]) -> Dict[int, List[str]]:
    ...
    if not kp_map: raise ValueError("no keypoints")
    return kp_map
...
def expand_pairs(segments_df: pd.DataFrame, keypoints_map: Dict[int, List[str]]) -> pd.DataFrame:
    ...
    for _, r in segments_df.iterrows():
        art = int(r["gdpr_article_number"]); kps = keypoints_map.get(art)
        if not kps: continue
    ...
    if df.empty: raise ValueError("no pairs")
    return df
...
def encode_one(tok, messages: List[Dict[str, str]]) -> Tuple[torch.Tensor, torch.Tensor]:
    try:
        enc = tok.apply_chat_template(messages, add_generation_prompt=True, return_tensors="pt"); input_ids = enc; attn_mask = torch.ones_like(input_ids)
    except Exception:
        txt = f"<s>[INST] {messages[0]['content']} [/INST]"; enc = tok(txt, return_tensors="pt"); input_ids, attn_mask = enc["input_ids"], enc["attention_mask"]
...
def extract_label(txt: str) -> str:
    try:
        ...
        m = LABEL_RE.search(txt or ""); lab = m.group(1).upper() if m else "UNKNOWN"
    return lab if lab in VALID else "NOT_APPLICABLE"

def generate_batched(model, tok, rows: pd.DataFrame, batch_size: int) -> List[str]:
    preds: List[str] = []; cache_ids: List[torch.Tensor] = []; cache_masks: List[torch.Tensor] = []
    def flush():
        ...
        ids = ids.to(DEVICE, non_blocking=True); att = att.to(DEVICE, non_blocking=True)
        with torch.inference_mode():
            out = model.generate(ids, attention_mask=att, max_new_tokens=GEN_MAX_NEW_TOK, do_sample=False, temperature=GEN_TEMPERATURE, top_p=GEN_TOP_P, pad_token_id=tok.pad_token_id, eos_token_id=tok.eos_token_id, use_cache=True)
        prompt_len = ids.shape[1]; out = out.to("cpu")
        for bi in range(out.size(0)):
            gen = out[bi, prompt_len:]; text = tok.decode(gen, skip_special_tokens=True); preds.append(extract_label(text))
        del ids, att, out; cache_ids.clear(); cache_masks.clear()
    for _, r in tqdm(rows.iterrows(), total=len(rows), desc="generation"):
        messages = build_messages(excerpt=r["segment"], art=int(r["gdpr_article_number"]), kp_id=str(r["keypoint_index"]), kp_text=r["keypoint_text"])
        input_ids, attn = encode_one(tok, messages); cache_ids.append(input_ids); cache_masks.append(attn)
        if len(cache_ids) >= batch_size: flush()
    flush(); return preds
...
def save_confusion(y_true, y_pred, labels, fname):
    ...
    fig, ax = plt.subplots(figsize=(6, 5)); ax.imshow(cm, interpolation="nearest")
    ...
    path = os.path.join(EVAL_OUT_DIR, fname); plt.savefig(path, dpi=220); plt.close()
    return cm.tolist()

def per_article_f1(article_ids, y_true, y_pred) -> pd.DataFrame:
    df_tmp = pd.DataFrame({"article": article_ids, "y_true": y_true, "y_pred": y_pred}); rows = []
...
def save_per_article_bar(df_scores: pd.DataFrame, fname_png: str, title: str):
    ...
    df_sorted = df_scores.sort_values("article"); fig_h = max(6, 0.18 * len(df_sorted))
    fig, ax = plt.subplots(figsize=(7, fig_h)); y = df_sorted["article"].astype(str).values; x = df_sorted["macro_f1"].values
    ax.barh(y, x); ax.set_xlim(0, 1.0); ax.set_xlabel("Per-Article Macro-F1"); ax.set_ylabel("GDPR Article"); ax.set_title(title); plt.tight_layout()
    path = os.path.join(EVAL_OUT_DIR, fname_png); plt.savefig(path, dpi=220); plt.close(); return path

def evaluate_with_adapter(adapter_path: str):
    segments_df = load_segments_flat(SEGMENTS_FILES); keypoints_map = load_keypoints(KEYPOINTS_FILES); pairs_df = expand_pairs(segments_df, keypoints_map)
    tok = AutoTokenizer.from_pretrained(BASE_MODEL, use_fast=True)
    if tok.pad_token is None: tok.pad_token = tok.eos_token
    tok.padding_side = "left"
    if not getattr(tok, "chat_template", None):
        tok.chat_template = ("<s>{% for m in messages %}{% if m['role']=='user' %}[INST] {{ m['content'] }} [/INST]{% elif m['role']=='assistant' %} {{ m['content'] }}{% endif %}{% endfor %}")
    base = AutoModelForCausalLM.from_pretrained(BASE_MODEL, load_in_8bit=True, device_map="auto")
    ...
    pred_pairs = pairs_df.copy(); pred_pairs["pred_label"] = preds
    ...
    if pred_pairs["policy_id"].notna().any(): group_keys = ["policy_id", "gdpr_article_number"]
    else: group_keys = ["segment_id", "gdpr_article_number"]
    def aggregate_labels(labels: List[str]) -> str:
        if all(l == "NOT_APPLICABLE" for l in labels): return "NOT_APPLICABLE"
        share_c = sum(l == "COMPLIANT" for l in labels) / len(labels); return "COMPLIANT" if share_c > 0.5 else "NON_COMPLIANT"
    agg_rows = []
    for _, grp in pred_pairs.groupby(group_keys):
        gold = aggregate_labels(grp["gold_label"].tolist()); pred = aggregate_labels(grp["pred_label"].tolist())
    ...
    scores_by_article = per_article_f1(article_ids=agg_df["article"].tolist(), y_true=agg_df["gold_article_label"].tolist(), y_pred=agg_df["pred_article_label"].tolist())
    ...
    with open(os.path.join(EVAL_OUT_DIR, "metrics.json"), "w", encoding="utf-8") as f:
        json.dump(metrics_json, f, indent=2, ensure_ascii=False)
    free_cuda()

def main():
    last_ckpt = train_lora_return_last_ckpt()
    evaluate_with_adapter(last_ckpt)
